...
export class NzModalWrapService {
  constructor(
    ...
    public modalDragService: ModalDragService
  ) {}
  ...
  protected createModalWidthDrag<T, R = NzSafeAny>(
    ...
    modalRef.afterOpen.subscribe(() => {
      const drag = this.modalDragService.createDragHandler(
        wrapCls,
        newConfig.nzModalType
      );
  ...
  protected createModalConfig<T, R = NzSafeAny>(
    config: ModalOptions<T, R>,
    wrapCls: string
  ): ModalOptions<T, R> {
    ...
    const maskStyle =
      config.nzMask === false ? { nzMaskStyle: { display: 'none' } } : {};
