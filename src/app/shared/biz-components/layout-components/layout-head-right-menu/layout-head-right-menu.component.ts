...
export class LayoutHeadRightMenuComponent implements OnInit {
  ...
  // 锁定屏幕
  lockScreen(): void {
    const i18nTitle = this.translate.instant('WIDGETS.LOCKSCREEN.TITLE');
  ...
  changePassWorld(): void {
    const i18nTitle = this.translate.instant('WIDGETS.CHANGE_PASSWORD.TITLE');
    this.changePasswordModalService.show({ nzTitle: i18nTitle }).subscribe(({ modalValue, status }) => {
